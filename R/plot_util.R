...
PlotEpi = function(simoutput,add=FALSE,showd=TRUE,showv=TRUE, ...)
  {
    mycols = c(rgb(55/255,126/255,184/255),rgb(228/255,26/255,28/255),rgb(77/255,175/255,74/255),rgb(152/255,78/255,163/255),rgb(255/255,127/255,0/255))
    if(add)
      {
        mylty=2
    ...
    if(showd & showv)
      {
        legend("topright",c("S","I","R","D","V"),fill=mycols)#,bg=rgb(1,1,1))
      }
    if(showd & !showv)
      {
        legend("topright",c("S","I","R","D"),fill=mycols[1:4])#,bg=rgb(1,1,1))
      }
    if(showv & !showd)
      {
        legend("topright",c("S","I","R","V"),fill=mycols[c(1:3,5)])#,bg=rgb(1,1,1))
      }
    if(!showd & !showv)
      {
        legend("topright",c("S","I","R"),fill=mycols[1:3])#,bg=rgb(1,1,1))
      }
  }
...
PlotParams <-  function(samp, dens, true) 
  {
    ##true <- drop(true)
    ...
    for(p in 1:4)
      {    
        ...
        plot(p.dens,bty='n',xlab=allnames[p],ylab="density",main="",ylim=myylim,type='n')
...
TimeSeriesOfDensities = function(X,times,xbounds,varname="",main="")
  {
    ...
    xrange = c(min(times),max(times)+1)#c(0,t)
